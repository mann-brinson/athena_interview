#Compute the N_ERRORS, and INFLOW grouped by month,contextid,errortype
WITH inflow AS (SELECT MONTH, CONTEXTID, ERRORTYPE,
    ...
    GROUP BY MONTH, CONTEXTID, ERRORTYPE
    ),

#Context/Errortype-level: Computes PREV_INFLOW for each contextid,errortype ordered by month
#Athena-level: Computes ATHENA_MONTH_INFLOW for each month
prev_inflow AS (SELECT *,
    IFNULL(LAG(inflow.INFLOW) OVER (PARTITION BY inflow.CONTEXTID, inflow.ERRORTYPE ORDER BY inflow.MONTH ASC), NULL) AS PREV_INFLOW,
    ROUND((AVG(inflow.INFLOW) OVER (PARTITION BY MONTH)), 6) ATHENA_MONTH_INFLOW
    FROM inflow
    ),

#Context/Errortype-level: Computes D_INFLOW for each month,contextid,errrortype
...
d_inflow AS (SELECT *,
    ...
    IFNULL(LAG(ATHENA_MONTH_INFLOW) OVER (PARTITION BY CONTEXTID, ERRORTYPE ORDER BY MONTH ASC), NULL) AS ATHENA_MONTH_INFLOW_PREV
    FROM prev_inflow
    )
...
AND d_inflow.D_INFLOW > 100 #Param
